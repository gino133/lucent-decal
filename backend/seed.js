...
async function run() {
  ...
  await Page.create({
    ...
    blocks: [
      { type: "richtext", order: 0, data: { html: "<p>Lucent Glass tôn trọng và bảo vệ thông tin cá nhân của khách hàng. Mọi dữ liệu thu thập qua form liên hệ, đặt hàng chỉ được sử dụng để tư vấn, xử lý đơn hàng và không chia sẻ cho bên thứ ba khi chưa có sự đồng ý của khách hàng.</p>" } },
    ],
  });
  ...
  await Page.create({
    ...
    blocks: [
      { type: "richtext", order: 0, data: { html: "<p>Khi sử dụng website và dịch vụ của Lucent Glass, khách hàng đồng ý với các điều khoản về đặt hàng, thanh toán, bảo hành và khiếu nại được quy định tại đây. Vui lòng liên hệ bộ phận CSKH để biết thêm chi tiết.</p>" } },
    ],
  });
