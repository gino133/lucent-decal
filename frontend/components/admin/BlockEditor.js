...
export default function BlockEditor({ block, onChange, onRemove }) {
  ...
  return (
    <div className="border border-gray-200 rounded-xl p-5 bg-gray-50 mb-4">
      ...
      {block.type === "richtext" && (
        <textarea rows={5} placeholder="Nội dung HTML" value={data.html || ""} onChange={(e) => updateData("html", e.target.value)} className="w-full border rounded-lg px-3 py-2 text-sm font-mono" />
      )}

      {block.type === "imageText" && (
        <div className="space-y-3">
          <input placeholder="Tiêu đề" value={data.title || ""} onChange={(e) => updateData("title", e.target.value)} className="w-full border rounded-lg px-3 py-2 text-sm" />
          <textarea rows={4} placeholder="Nội dung HTML" value={data.html || ""} onChange={(e) => updateData("html", e.target.value)} className="w-full border rounded-lg px-3 py-2 text-sm font-mono" />
