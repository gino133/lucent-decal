...
export default function DashboardPage() {
  const [stats, setStats] = useState({ products: 0, projects: 0, orders: 0, contacts: 0, categories: 0 });

  useEffect(() => {
    async function load() {
      try {
        const [p, pr, o, c, cat] = await Promise.all([
    ...
    <div>
      ...
      <div className="bg-white rounded-xl p-8 mt-8">
        ...
        <ul className="text-sm text-gray-600 space-y-2 list-disc pl-5">
          ...
          <li>Vào <strong>Menu điều hướng</strong> để thêm/sửa/xoá các trang con hiển thị trên thanh menu.</li>
          <li>Vào <strong>Nội dung trang</strong> để chỉnh sửa từng khối nội dung của Trang chủ, Giới thiệu, Hồ sơ năng lực, Liên hệ.</li>
